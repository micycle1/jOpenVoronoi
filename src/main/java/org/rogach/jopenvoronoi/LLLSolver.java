...
//  where A_i is A with column i replaced by b
	...
	public int solve(Site s1, double k1, Site s2, double k2, Site s3, double k3, List<Solution> slns) {
		...
		int i = 0, j = 1, k = 2;
		double d = chop(determinant(eq.get(i).a, eq.get(i).b, eq.get(i).k, eq.get(j).a, eq.get(j).b, eq.get(j).k,
				eq.get(k).a, eq.get(k).b, eq.get(k).k));
		double det_eps = 1e-6;
		if (Math.abs(d) > det_eps) {
			double t = determinant(eq.get(i).a, eq.get(i).b, -eq.get(i).c, eq.get(j).a, eq.get(j).b, -eq.get(j).c,
					eq.get(k).a, eq.get(k).b, -eq.get(k).c) / d;
			if (t >= 0) {
				double sol_x = determinant(-eq.get(i).c, eq.get(i).b, eq.get(i).k, -eq.get(j).c, eq.get(j).b,
						eq.get(j).k, -eq.get(k).c, eq.get(k).b, eq.get(k).k) / d;
				double sol_y = determinant(eq.get(i).a, -eq.get(i).c, eq.get(i).k, eq.get(j).a, -eq.get(j).c,
						eq.get(j).k, eq.get(k).a, -eq.get(k).c, eq.get(k).k) / d;
