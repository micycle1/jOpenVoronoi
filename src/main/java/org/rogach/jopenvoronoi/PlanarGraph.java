...
public class PlanarGraph {
		...
		try (PrintWriter w = new PrintWriter(f)) {
			...
			for (Segment s : segments) {
				w.printf("<polyline points='%f,%f %f,%f' fill='none' stroke-width='1' stroke='rgb(255,255,0)'/>\n",
						s.stt.getX() * 512 + 512, -s.stt.getY() * 512 + 512, s.end.getX() * 512 + 512,
						-s.end.getY() * 512 + 512);
			}
			for (Point2D p : points) {
	...
	public void writeToFile(String fname) throws Exception {
		File f = new File(fname);
		f.getAbsoluteFile().getParentFile().mkdirs();
		try (OutputStream os = new FileOutputStream(f); GZIPOutputStream gzos = new GZIPOutputStream(os)) {
			int i = 0;
	...
	public static PlanarGraph readFromFile(String fname) throws Exception {
		try (FileInputStream fis = new FileInputStream(fname); GZIPInputStream gzis = new GZIPInputStream(fis)) {
			DocumentBuilderFactory documentBuilderFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder documentBuilder = documentBuilderFactory.newDocumentBuilder();
			...
			for (int q = 0; q < pointSites.getLength(); q++) {
				...
				Point2D p = new Point2D.Double(Double.valueOf(ps.getAttributes().getNamedItem("x").getTextContent()),
			...
			for (int q = 0; q < lineSites.getLength(); q++) {
				...
				segments.add(new Segment(
						pointIndices
								.get(Integer.valueOf(ls.getAttributes().getNamedItem("source_idx").getTextContent())),
						pointIndices
								.get(Integer.valueOf(ls.getAttributes().getNamedItem("target_idx").getTextContent()))));
			}
