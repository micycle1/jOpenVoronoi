...
public class RandomPolygon {
	...
	public static List<Point2D> generate_points(int nPoints, int max) {
		List<Point2D> points = new ArrayList<>();
		for (int i = 0; i < nPoints; i++) {
			points.add(new Point2D.Double(Math.random() * max, Math.random() * max));
		}
		return points;
	}

	public static List<Point2D> generate_polygon(int nPoints, int max) {
		List<Point2D> points = generate_points(nPoints, max);
	...
	public static void partition(List<Point2D> points, Point2D p1, Point2D p2, List<Point2D> group1,
			List<Point2D> group2) {
